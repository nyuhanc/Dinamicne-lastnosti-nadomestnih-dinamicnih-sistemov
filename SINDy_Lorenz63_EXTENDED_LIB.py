...
# Lorenz model
def lorenz_modified(t, x, sigma=10, beta=2.66667, rho=28):
    return [
        sigma * (x[1] - x[0]) + 1,
        x[0] * (rho - x[2]) - x[1] + 1,
        x[0] * x[1] - beta * x[2] + 1,
...
t_span = (t[0], t[-1])
x0 = [1, -1, 20]
x_train = solve_ivp(lorenz_modified, t_span, x0, t_eval=t, **integrator_keywords).y.T
x0 = [-13.83058727,   4.62887942,   8.34341018] #(np.random.rand(3) - 0.5) * 30
x_test = solve_ivp(lorenz_modified, t_span, x0, t_eval=t, **integrator_keywords).y.T
