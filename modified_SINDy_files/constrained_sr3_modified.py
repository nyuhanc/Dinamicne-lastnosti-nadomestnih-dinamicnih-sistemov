...
class ConstrainedSR3(SR3):
    ...
    def _objective(self, x, y, q, coef_full, coef_sparse, trimming_array=None):
        ...
        if self.thresholds is None:
            regularization = get_regularization(self.thresholder)(coef_full, self.threshold**2 / self.nu) #self.reg(coef_full, self.threshold**2 / self.nu)
            if print_ind == 0 and self.verbose:
                ...
                print(
                    "{0:10.4e} ... {1:10.4e} ... {2:10.4e}"
                    " ... {3:10.4e}".format(*row)
                )
            return 0.5 * np.sum(R2) + 0.5 * regularization + 0.5 * np.sum(D2) / self.nu
        else:
            regularization = get_regularization(self.thresholder)(coef_full, self.thresholds**2 / self.nu)#self.reg(coef_full, self.thresholds**2 / self.nu)
            if print_ind == 0 and self.verbose:
                row = [
                    "|y - Xw|^2",
                    "|w-u|^2/v",
    ...
    def _reduce(self, x, y):
        ...
        if self.inequality_constraints:
            ...
            for k in range(self.max_iter):
                ...
                #print('Iteration: {0:>10d}/{1:<10d}'.format(k + 1, self.max_iter))
