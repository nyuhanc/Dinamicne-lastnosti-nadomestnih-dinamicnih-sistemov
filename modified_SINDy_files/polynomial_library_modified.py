...
class PolynomialLibrary(PolynomialFeatures, BaseFeatureLibrary):
    ...
    def _combinations(
        ...
        if degree == 2: # we rearrange the library so that it works with constraints
            C = [j for j in chain.from_iterable(comb(range(n_features), i) for i in range(start, degree + 1))]
            D = [1 for _ in range(len(C))]
            r = n_features
